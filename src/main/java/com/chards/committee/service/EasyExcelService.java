...
//        exportHead.add(TeachStaffExportHead.ExcelHead5());
     ...
     * @param listener  监听器
     * @param dataClass 读取的数据
     * @param <T>
     * @throws IOException
     */
    public <T> void readToReponse(MultipartFile file, ExcelDataListener listener,Class<T> dataClass) throws IOException {
        EasyExcel.read(file.getInputStream(),dataClass,listener).sheet().doRead();
    }
